...
export const askAIAboutNotesAction = async (newQuestions: string[], responses: string[]) => {
        ...
        if (!user) {
            throw new Error("User must be logged in to chat with Biggie.");
        }
        ...
        if (notes.length === 0) {
            return "You have no notes for Biggie to analyze.";
        }
        ...
        return completion.choices[0].message.content || "Sorry bro, I couldn't come up with an answer.";
