...
function AskAIButton({ user }: Props) {
  ...
  const handleOpOpenChange = (isOpen: boolean) => {
    if (!user) {
      router.push('/login')
    } else {
      if (isOpen) {
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpOpenChange}>
      <form>
        <DialogTrigger asChild>
          <Button variant="secondary">Ask Biggy</Button>
        </DialogTrigger>
        <DialogContent
        className="custom-scrollbar flex h-[85vh] max-w-4xl flex-col overflow-y-auto"
        ref={contentRef}>
          <DialogHeader>
            <DialogTitle>Ask Biggie about your notes</DialogTitle>
            <DialogDescription>
              Yo! Ask Notorious anything about your notes and he'll help you B.I.G. time!
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="mt-auto flex cursor-text flex-col rounded-lg border p-4" onClick={handleClickInput}>
            <Textarea ref={textareaRef}
              placeholder="What you wanna know about your notes, bro?"
